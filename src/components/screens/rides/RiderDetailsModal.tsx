...
import { acceptRider, acceptRidersKyc, rejectRider, rejectRidersKyc } from "@/apis/rides";
...
export function RiderDetailsModal({
  ...
  const [KycisSubmitting, setIsKycSubmitting] = useState(false);
  ...
  const handleKycReject = async () => {
    ...
    try {
      ...
      await rejectRidersKyc(riderId,token as string, rejectionReason)
      setActionStatus({
        type: "success",
  ...
  const handleApprove = async () => {
    try {
      ...
      await acceptRider(riderId, token as string);
      setActionStatus({
        type: "success",
  ...
  const handleKycApprove = async () => {
    try {
      ...
      setActionStatus(null);
      // TODO: Call your API to approve the rider
      // await approveRider(rider?.userId);
      console.log(rider?.userId)
     await acceptRidersKyc(riderId,token as string)
      setActionStatus({
        type: "success",
      ...
      setActionStatus({ type: "error", message: "Failed to approve KYC" });
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await rejectRider(rider?.userId, token as string, rejectionReason);
                  ...
                  {!isKycRejecting ? (
                    <>
                      ...
                      <button
                        ...
                        disabled={KycisSubmitting}
                      >
                        {KycisSubmitting ? "Approving..." : "Approve KYC"}
                      </button>
                    </>
                  ) : (
